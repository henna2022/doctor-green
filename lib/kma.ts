// ━━━ 기상청(KMA) 격자변환 / base_time 계산 / 응답 파싱 ━━━
// app/api/kma/route.ts 에서 쓰던 순수 함수들을 분리.
// (route.ts는 Next.js 규칙상 HTTP 메서드 외의 export를 허용하지 않아 단위 테스트가 불가능했음)

// 위경도 → 기상청 격자 변환
export function latLonToKmaGrid(lat: number, lon: number) {
...
export function getKmaBaseTime() {
